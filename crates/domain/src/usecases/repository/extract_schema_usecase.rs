...
impl<R: DatabaseProviderRegistry> ExtractSchemaUseCase<R> {
    ...
    pub async fn run(&self, path: &Path) -> Result<SchemaOutput, ExtractSchemaError> {
        ...
        // 3. Connection info for the env (credentials, db name) from the running
        //    container. The command runs *inside* that container via `exec`, so it
        //    reaches the database on 127.0.0.1 rather than the container's network IP.
        let conn_info = self
            .compute
        ...
        // 4. Get schema extraction spec (a shell command that prints delimited output).
        let spec = provider
        ...
        //    the client tools the command needs (psql), so no separate container,
        //    image pull, or network linking is required.
